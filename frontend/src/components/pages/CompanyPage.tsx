...
const CompanyPage: FC = () =>
{
    ...
    const [showContractorCompanyCreateForm, setShowContractorCompanyCreateForm] = useState<boolean>(false);
    const [showContractorPersonCreateForm, setShowContractorPersonCreateForm] = useState<boolean>(false);
    const [showExecutorPersonCreateForm, setShowExecutorPersonCreateForm] = useState<boolean>(false);
    ...
    return (<div>
        ...
        <List items={contractorCompanies.map((contractor, index) =>
            ({id: index, content: <div>{contractor.companyFullName}</div>} as ListItem))}
              onAdd={() => setShowContractorCompanyCreateForm(!showContractorCompanyCreateForm)}/>

        {showContractorCompanyCreateForm ? <Form submitLabel={"Добавить компанию заказчика"}
            inputs={contractorCompanyFields.map(field => ({inputData: field} as InputProps))}
            onSubmit={requestCreateContractorCompany}/> : null}
        ...
        onAdd={() => setShowContractorPersonCreateForm(!showContractorPersonCreateForm)}/>

        {showContractorPersonCreateForm ? <Form submitLabel={"Добавить юр. лицо заказчика"}
            inputs={contractorPersonFields.map(field => ({inputData: field} as InputProps))}
            onSubmit={requestCreateContractorPerson}/> : null}
        ...
        <List items={executorPersons.map((person, index) =>
            ({id: index, content: <div>{person.post + " " + person.firstName}</div>} as ListItem))}
              onAdd={() => setShowExecutorPersonCreateForm(!showExecutorPersonCreateForm)}/>

        {showExecutorPersonCreateForm ? <Form submitLabel={"Добавить юр. лицо исполнителя"}
            inputs={executorPersonFields.map(field => ({inputData: field} as InputProps))}
            onSubmit={requestCreateExecutorPerson}/> : null}
